...
void LogCrypt::UpdateLogHour(char *_data) {
  ...
  memcpy(_data + GetHeaderLen() - sizeof(uint32_t) - sizeof(char) * 64 - sizeof(char),
         &hour,
         sizeof(hour));
}
...
bool LogCrypt::GetPeriodLogs(const char *const _log_path,
  ...
  if (NULL == _log_path || _end_hour <= _begin_hour) {
    snprintf(msg,
             sizeof(msg),
             "NULL == _logPath || _endHour <= _beginHour, %d, %d",
             _begin_hour,
             _end_hour);
    return false;
  }
  ...
  if (!ret) {
    ...
    snprintf(msg,
             sizeof(msg),
             "begintpos:%lu, endpos:%lu, filesize:%ld.",
             _begin_pos,
             _end_pos,
             file_size);
    _err_msg += msg;
...
void LogCrypt::CryptAsyncLog(const char *const _log_data,
  ...
  memcpy((char *) _out_buff.Ptr() + _input_len - _remain_nocrypt_len,
         _log_data + _input_len - _remain_nocrypt_len,
         _remain_nocrypt_len);
#endif
}
