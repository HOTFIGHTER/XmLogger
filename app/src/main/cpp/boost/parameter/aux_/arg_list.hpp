...
struct arg_list: Next {
  ...
  typedef typename mpl::eval_if<
      holds_maybe, get_reference<typename TaggedArg::value_type>, get_reference<TaggedArg>
  >::type reference;

  typedef typename mpl::if_<
      holds_maybe, reference, typename TaggedArg::value_type
  >::type value_type;
  ...
  BOOST_PP_ENUM_BINARY_PARAMS(BOOST_PARAMETER_MAX_ARITY, A, &
  a)
  )
  :
  Next( // a1, a2, ...
  ...
  struct binding {
    ...
    struct apply {
      typedef typename mpl::eval_if<
          mars_boost::is_same<KW, key_type>, mpl::if_ < Reference, reference, value_type>
  ...
  static typename mpl::apply_wrap2<
      typename mpl::lambda<Predicate, lambda_tag>::type, value_type, ArgPack
  >::type
  satisfies(
      parameter_requirements <key_type, Predicate, HasDefault> *, ArgPack *
  );
