...
struct has_nothrow_assign_imp { static const bool value = false; };
template<class T>
struct has_nothrow_assign_imp<T, false, true> {
  static const bool value = noexcept(mars_boost::declval<typename add_reference<T>::type>() =
                                         mars_boost::declval<typename add_reference<T const>::type>());
};
