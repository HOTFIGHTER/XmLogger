...
struct thread_safe_counter {
  ...
  static unsigned int load(mars_boost::detail::atomic_count const &counter)
  BOOST_NOEXCEPT
  {
    return static_cast< unsigned int >(static_cast< long >(counter));
  }

  static void increment(mars_boost::detail::atomic_count &counter)
  BOOST_NOEXCEPT
  {
    ++counter;
  }

  static unsigned int decrement(mars_boost::detail::atomic_count &counter)
  BOOST_NOEXCEPT
  {
    return --counter;
  }
...
class intrusive_ref_counter {
 ...
 public:
  ...
  BOOST_NOEXCEPT : m_ref_counter(0) {
  }
 ...
 protected:
  ...
  friend void intrusive_ptr_add_ref<DerivedT, CounterPolicyT>(const intrusive_ref_counter<DerivedT,
                                                                                          CounterPolicyT> *p)
  BOOST_NOEXCEPT;
  friend void intrusive_ptr_release<DerivedT, CounterPolicyT>(const intrusive_ref_counter<DerivedT,
                                                                                          CounterPolicyT> *p)
  BOOST_NOEXCEPT;
};
