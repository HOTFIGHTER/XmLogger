...
namespace mars_boost {
template<typename T>
mars_boost::shared_ptr<T> shared_from_raw(T *);
template<typename T>
mars_boost::weak_ptr<T> weak_from_raw(T *);
...
class enable_shared_from_raw {
 ...
 private:
  ...
  class shared_ptr;
  template<typename T>
  friend mars_boost::shared_ptr<T> shared_from_raw(T *);
  template<typename T>
  friend mars_boost::weak_ptr<T> weak_from_raw(T *);
  template<class X, class Y>
  friend inline void detail::sp_enable_shared_from_this(mars_boost::shared_ptr<X> *ppx,
                                                        Y const *py,
                                                        mars_boost::enable_shared_from_raw const *pe);
#endif
  ...
  void _internal_accept_owner(shared_ptr<X> *ppx, Y *py) const {
    ...
    if (weak_this_.expired()) {
      ...
      detail::esft2_deleter_wrapper
          *pd = mars_boost::get_deleter<detail::esft2_deleter_wrapper>(shared_this_);
      BOOST_ASSERT(pd != 0);
