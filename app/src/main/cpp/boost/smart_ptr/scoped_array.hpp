...
class scoped_array // noncopyable
{
 ...
 public:
  ...
  void
  reset(T *p = 0) // never throws (but has a BOOST_ASSERT in it, so not marked with BOOST_NOEXCEPT)
  {
...
template<class T>
inline void swap(scoped_array<T> &a, scoped_array<T> &b) BOOST_NOEXCEPT {
  a.swap(b);
}
