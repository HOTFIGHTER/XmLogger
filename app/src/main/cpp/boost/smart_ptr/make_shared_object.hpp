...
class sp_ms_deleter {
 ...
 public:
  ...
  sp_ms_deleter()
  BOOST_NOEXCEPT : initialized_(false) {
  }

  template<class A>
  explicit sp_ms_deleter(A const &)
  BOOST_NOEXCEPT : initialized_(false) {
  }
...
class sp_as_deleter {
 ...
 public:
  ...
  sp_as_deleter(A const &a)
  BOOST_NOEXCEPT : a_(a), initialized_(
  false )
  {
  }

  // optimization: do not copy storage_
  sp_as_deleter(sp_as_deleter const &r)
  BOOST_NOEXCEPT : a_(r.a_), initialized_(
  false )
  {
  }
...
template<class T>
typename mars_boost::detail::sp_if_not_array<T>::type make_shared_noinit() {
  mars_boost::shared_ptr<T> pt(static_cast< T * >( 0 ), BOOST_SP_MSD(T));

  mars_boost::detail::sp_ms_deleter<T>
      *pd = static_cast<boost::detail::sp_ms_deleter<T> *>( pt._internal_get_untyped_deleter());
...
template<class T, class A>
typename mars_boost::detail::sp_if_not_array<T>::type allocate_shared_noinit(A const &a) {
...
typename mars_boost::detail::sp_if_not_array<T>::type make_shared(Args &&... args) {
  mars_boost::shared_ptr<T> pt(static_cast< T * >( 0 ), BOOST_SP_MSD(T));
...
typename mars_boost::detail::sp_if_not_array<T>::type allocate_shared(A const &a, Args &&... args) {
#if !defined( BOOST_NO_CXX11_ALLOCATOR )
  ...
  mars_boost::shared_ptr<T>
      pt(static_cast< T * >( 0 ), mars_boost::detail::sp_inplace_tag<D>(), a2);
  ...
  std::allocator_traits<A2>::construct(a2,
                                       static_cast< T * >( pv ),
                                       mars_boost::detail::sp_forward<Args>(args)...);
