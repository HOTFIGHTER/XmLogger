...
inline uintptr_t
_beginthreadex(void *security, unsigned stack_size, unsigned (__stdcall *start_address)(void *),
               void *arglist, unsigned initflag, unsigned *thrdaddr) {
  DWORD threadID;
  ThreadProxyData *data = new ThreadProxyData(start_address, arglist);
  HANDLE
      hthread = CreateThread(static_cast<LPSECURITY_ATTRIBUTES>(security), stack_size, ThreadProxy,
    ...
    while (!current_thread_data->tss_data.empty() || current_thread_data->thread_exit_callbacks) {
      while (current_thread_data->thread_exit_callbacks) {
        detail::thread_exit_callback_node
            *const current_node = current_thread_data->thread_exit_callbacks;
...
//           mars_boost::throw_exception(thread_resource_error());
  ...
  return true;
#else
  uintptr_t const new_thread = _beginthreadex(0,
                                              0,
...
bool thread::start_thread_noexcept(const attributes &attr) {
  ...
  uintptr_t const new_thread = _beginthreadex(0,
                                              static_cast<unsigned int>(attr.get_stack_size()),
                                              &thread_start_function,
                                              thread_info.get(),
                                              CREATE_SUSPENDED,
                                              &thread_info->id);
...
unsigned thread::physical_concurrency()
BOOST_NOEXCEPT
{
...
if (
GetLogicalProcessorInformation(&buffer
.
front(), &size
) == FALSE)
return 0;
...
LARGE_INTEGER get_due_time(detail::timeout const &target_time) {
  ...
  if (target_time.relative) {
    detail::win32::ticks_type const
        elapsed_milliseconds = detail::win32::GetTickCount64_()() - target_time.start;
...
bool interruptible_wait(detail::win32::handle handle_to_wait_for, detail::timeout target_time) {
  ...
  do {
    ...
    if (handle_count) {
      unsigned long const notified_index = detail::win32::WaitForMultipleObjectsEx(handle_count,
                                                                                   handles,
                                                                                   false,
                                                                                   using_timer
                                                                                   ? INFINITE
...
#else
return
thread::id (get_or_make_current_thread_data());
#endif
}
