...
if (f.
compare_exchange_strong(expected, in_progress, memory_order_acq_rel, memory_order_acquire
)) {
...
//pthread::pthread_mutex_scoped_lock lk(&once_mutex);
BOOST_VERIFY(!
pthread_cond_wait(&once_cv, &once_mutex
));
}
}
}
...
pthread::pthread_mutex_scoped_lock lk(&once_mutex);
f.
store(initialized, memory_order_release
);
}
BOOST_VERIFY(!
...
pthread::pthread_mutex_scoped_lock lk(&once_mutex);
f.
store(uninitialized, memory_order_release
);
}
BOOST_VERIFY(!
pthread_cond_broadcast(&once_cv)
