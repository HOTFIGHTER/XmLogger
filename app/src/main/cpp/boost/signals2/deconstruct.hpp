...
class deconstruct_deleter {
 private:
  ...
  typedef typename sp_aligned_storage<sizeof(T), ::mars_boost::alignment_of<T>::value>::type
      storage_type;
...
class deconstruct_access {
 public:
  ...
  static postconstructor_invoker<T> deconstruct() {
    ...
    detail::deconstruct_deleter<T>
        *pd = mars_boost::get_deleter<detail::deconstruct_deleter<T> >(pt);
