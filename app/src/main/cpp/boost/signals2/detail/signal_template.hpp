...
template<typename R>
class BOOST_SIGNALS2_BOUND_EXTENDED_SLOT_FUNCTION_INVOKER_N(
...
class BOOST_SIGNALS2_SIGNAL_IMPL_CLASS_NAME(
BOOST_SIGNALS2_NUM_ARGS) BOOST_SIGNALS2_SIGNAL_TEMPLATE_SPECIALIZATION
{
public:
...
typedef BOOST_SIGNALS2_EXTENDED_SLOT_TYPE(BOOST_SIGNALS2_NUM_ARGS)
extended_slot_type;
...
typedef slot_call_iterator_cache <nonvoid_slot_result_type, slot_invoker>
    slot_call_iterator_cache_type;
...
typedef typename detail::slot_call_iterator_t<slot_invoker,
                                              ...
                                              connection_body < group_key_type,
                                              slot_type,
                                              Mutex> >
slot_call_iterator;
...
_shared_state(new
invocation_state(connection_list_type(group_compare), combiner_arg
)),
_garbage_collector_it(_shared_state
->
connection_bodies()
.
end()
...
connection
connect_extended(const extended_slot_type &ext_slot, connect_position position = at_back) {
  garbage_collecting_lock <mutex_type> lock(*_mutex);
...
nolock_cleanup_connections(list_lock,
false, 1);
/* Make a local copy of _shared_state while holding mutex, so we are
thread safe against the combiner or connection list getting modified
during invocation. */
...
invocation_janitor janitor(cache, *this, &local_state->connection_bodies());
return
detail::combiner_invoker<typename combiner_type::result_type>()
(
local_state->
combiner(),
    slot_call_iterator
    (local_state->connection_bodies().begin(), local_state->connection_bodies().end(), cache),
    slot_call_iterator
    (local_state->connection_bodies().end(), local_state->connection_bodies().end(), cache)
);
}
...
class invocation_state {
 public:
  invocation_state(const connection_list_type &connections_in,
                   const combiner_type &combiner_in) : _connection_bodies(new connection_list_type(
      connections_in)),
...
class invocation_janitor: noncopyable {
 public:
  typedef BOOST_SIGNALS2_SIGNAL_IMPL_CLASS_NAME(BOOST_SIGNALS2_NUM_ARGS)
  signal_type;
  invocation_janitor
      (
...
class BOOST_SIGNALS2_SIGNAL_CLASS_NAME(
BOOST_SIGNALS2_NUM_ARGS);
...
public
detail::BOOST_SIGNALS2_STD_FUNCTIONAL_BASE
