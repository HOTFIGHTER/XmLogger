...
template<class T>
inline const detail::ctti_data &ctti_construct()
BOOST_NOEXCEPT {
...
<const detail::ctti_data *> (mars_boost::detail::ctti<T>::n());
}
...
class ctti_type_index: public type_index_facade<ctti_type_index, detail::ctti_data> {
  ...
  inline std::size_t get_raw_name_length() const
  BOOST_NOEXCEPT;

 public:
  typedef detail::ctti_data type_info_t;

  inline ctti_type_index()
  BOOST_NOEXCEPT
      : data_(&ctti_construct<void>()) {}

  inline ctti_type_index(const type_info_t &data)
  BOOST_NOEXCEPT
      : data_(&data) {}
  ...
  template<class T>
  inline static ctti_type_index type_id()
  BOOST_NOEXCEPT;

  template<class T>
  inline static ctti_type_index type_id_with_cvr()
  BOOST_NOEXCEPT;

  template<class T>
  inline static ctti_type_index type_id_runtime(const T &variable)
  BOOST_NOEXCEPT;
};


inline const ctti_type_index::type_info_t &ctti_type_index::type_info() const
BOOST_NOEXCEPT {
return *
data_;
}
...
template<class T>
inline ctti_type_index ctti_type_index::type_id()
BOOST_NOEXCEPT {
typedef BOOST_DEDUCED_TYPENAME mars_boost
::remove_reference<T>::type no_ref_t;
typedef BOOST_DEDUCED_TYPENAME mars_boost
::remove_cv<no_ref_t>::type no_cvr_t;
return
ctti_construct<no_cvr_t>();
}


template<class T>
inline ctti_type_index ctti_type_index::type_id_with_cvr()
BOOST_NOEXCEPT {
return
ctti_construct<T>();
}
...
template<class T>
inline ctti_type_index ctti_type_index::type_id_runtime(const T &variable)
BOOST_NOEXCEPT {
return variable.
...
inline const char *ctti_type_index::raw_name() const
BOOST_NOEXCEPT {
return reinterpret_cast
<const char *>(data_);
}

inline std::size_t ctti_type_index::get_raw_name_length() const
BOOST_NOEXCEPT {
return
std::strlen (raw_name()
+ detail::ctti_skip_size_at_end);
}
...
inline std::size_t ctti_type_index::hash_code() const
BOOST_NOEXCEPT {
return
mars_boost::hash_range(raw_name(), raw_name()
+
get_raw_name_length()
);
}
