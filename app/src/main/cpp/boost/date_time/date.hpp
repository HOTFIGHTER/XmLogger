...
class date: private mars_boost::less_than_comparable<T, mars_boost::equality_comparable<T
> > {
 public:
  typedef T date_type;
  ...
  duration_type operator-(const date_type &d) const {
    if (!this->is_special() && !d.is_special()) {
      ...
      return duration_type(
          static_cast< duration_rep_type >(days_) - static_cast< duration_rep_type >(d.days_));
    } else {
