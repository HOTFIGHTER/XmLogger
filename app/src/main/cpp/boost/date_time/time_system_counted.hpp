...
struct counted_time_rep {
  ...
  date_type date() const {
    if (time_count_.is_special()) {
      ...
      typename calendar_type::date_int_type
          dc = static_cast<typename calendar_type::date_int_type>(day_count());
  ...
  unsigned long day_count() const {
     ...
     */
    return static_cast<unsigned long>(resolution_traits::as_number(time_count_)
        / frac_sec_per_day());
  }
