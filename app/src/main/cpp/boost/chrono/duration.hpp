...
typedef duration<mars_boost::int_least32_t, ratio < 60> >
minutes; // at least 29 bits needed
typedef duration<mars_boost::int_least32_t, ratio < 3600> >
...
struct duration_cast_aux<FromDuration, ToDuration, Period, true, true> {
  BOOST_CONSTEXPR ToDuration
  operator()(const FromDuration &fd) const {
...
struct chrono_numeric_limits {
  static BOOST_CHRONO_LIB_CONSTEXPR T
  lowest()
  BOOST_CHRONO_LIB_NOEXCEPT_OR_THROW { return (std::numeric_limits<T>::min)(); }
};
...
template<class Rep>
struct duration_values {
  static BOOST_CONSTEXPR Rep
  zero() { return Rep(0); }
  static BOOST_CHRONO_LIB_CONSTEXPR Rep
