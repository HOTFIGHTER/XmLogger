...
if ( !
mars_boost::detail::winapi::QueryPerformanceFrequency( &freq )
)
return 0.0L;
...
return
steady_clock::time_point(steady_clock::duration(
...
BOOST_CHRONO_INLINE
    system_clock::time_point
system_clock::now( system::error_code
& ec )
{
mars_boost::detail::winapi::FILETIME_ ft;
mars_boost::detail::winapi::GetSystemTimeAsFileTime( &ft );  // never fails
...
BOOST_CHRONO_INLINE
    std::time_t
system_clock::to_time_t(const system_clock::time_point &t)
BOOST_NOEXCEPT
{
...
BOOST_CHRONO_INLINE
    system_clock::time_point
system_clock::from_time_t(std::time_t
t) BOOST_NOEXCEPT
{
__int64 temp = t;
temp *= 10000000;
return
time_point(duration(temp)
);
}
