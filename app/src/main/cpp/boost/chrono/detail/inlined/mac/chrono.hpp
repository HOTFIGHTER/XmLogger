...
BOOST_NOEXCEPT {
return
time_t(duration_cast<seconds>(t.time_since_epoch())
.
count()
);
}
...
{
return
system_clock::time_point (seconds(t));
}
...
BOOST_CHRONO_STATIC
    steady_clock::rep
steady_simplified_ec(system::error_code
& ec) {
if (!
BOOST_CHRONO_IS_THROWS(ec)
) {
ec.
clear();
}
...
BOOST_CHRONO_STATIC
    steady_clock::rep
steady_full_ec(system::error_code
& ec)
{
kern_return_t err;
const double factor = chrono_detail::compute_steady_factor(err);
...
{
ec.
assign( errno, BOOST_CHRONO_SYSTEM_CATEGORY
);
return
steady_clock::rep();
}
...
BOOST_CHRONO_STATIC
    FP
init_steady_clock(kern_return_t
& err)
{
mach_timebase_info_data_t MachInfo;
err = mach_timebase_info(&MachInfo);
...
{
return &
chrono_detail::steady_simplified;
}
return &
chrono_detail::steady_full;
}

#if !defined BOOST_CHRONO_DONT_PROVIDE_HYBRID_ERROR_HANDLING
BOOST_CHRONO_STATIC
    FP_ec
init_steady_clock_ec(kern_return_t
& err)
{
mach_timebase_info_data_t MachInfo;
err = mach_timebase_info(&MachInfo);
...
{
return &
chrono_detail::steady_simplified_ec;
}
return &
chrono_detail::steady_full_ec;
}
#endif
}
