...
if (factor != -1) {
return
time_point (nanoseconds(c *factor));
}
else
{
BOOST_ASSERT(0 && "Boost::Chrono - Internal Error");
...
process_real_cpu_clock::time_point process_real_cpu_clock::now(system::error_code &ec) {
  ...
  if (c == clock_t(-1)) // error
  {
    if (BOOST_CHRONO_IS_THROWS(ec)) {
      mars_boost::throw_exception(system::system_error(errno,
...
{
return
time_point(nanoseconds((tm.tms_utime + tm.tms_cutime)
                           * factor)
);
} else
{
