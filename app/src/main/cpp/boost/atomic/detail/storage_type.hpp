...
non_atomic_load(buffer_storage<Size> const volatile &from, buffer_storage<Size> &to)
BOOST_NOEXCEPT {
BOOST_ATOMIC_DETAIL_MEMCPY(to
.data, const_cast
...
struct make_storage_type {
  ...
  struct aligned {
    ...
    explicit aligned(type const &v)
    BOOST_NOEXCEPT : value(v) {}
  };
};
...
BOOST_FORCEINLINE bool operator==(storage128_t const &left, storage128_t const &right)
BOOST_NOEXCEPT {
...
BOOST_FORCEINLINE void non_atomic_load(storage128_t const volatile &from, storage128_t &to)
BOOST_NOEXCEPT
{
