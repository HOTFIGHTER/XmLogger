...
class symmetric_coroutine_impl: private noncopyable {
 ...
 protected:
  template<typename X>
  friend
  class symmetric_coroutine_impl;

  int flags_;
  coroutine_context caller_;
  coroutine_context callee_;

  void resume_(param_type *to)
  BOOST_NOEXCEPT
  {
    BOOST_ASSERT(!is_running());
    BOOST_ASSERT(!is_complete());
...
class symmetric_coroutine_impl<void>: private noncopyable {
 public:
  ...
  inline bool force_unwind() const
  BOOST_NOEXCEPT
  { return 0 != (flags_ & flag_force_unwind); }

  inline bool unwind_requested() const
  BOOST_NOEXCEPT
  { return 0 != (flags_ & flag_unwind_stack); }
