...
struct scoped_deallocator {
 ...
 private:
  void priv_deallocate(version_1) { m_alloc.deallocate(m_ptr, 1); }

  void priv_deallocate(version_2) { m_alloc.deallocate_one(m_ptr); }
 ...
 public:
  ...
  ~scoped_deallocator() { if (m_ptr)priv_deallocate(alloc_version()); }

  scoped_deallocator (BOOST_RV_REF(scoped_deallocator)
  o)
  :
  m_ptr(o
  .m_ptr),
  ...
  void set(const pointer &p) { m_ptr = p; }
...
struct scoped_array_deallocator {
  ...
  ~scoped_array_deallocator() { if (m_ptr) m_alloc.deallocate(m_ptr, m_length); }
...
struct scoped_destroy_deallocator {
 ...
 private:
  ...
  void priv_deallocate(const pointer &p, version_1) { AllocTraits::deallocate(m_alloc, p, 1); }

  void priv_deallocate(const pointer &p, version_2) { m_alloc.deallocate_one(p); }
...
struct scoped_destructor_n {
  ...
  void increment_size(size_type inc) { m_n += inc; }
...
class allocator_destroyer {
 ...
 private:
  void priv_deallocate(const pointer &p, version_1) { AllocTraits::deallocate(a_, p, 1); }

  void priv_deallocate(const pointer &p, version_2) { a_.deallocate_one(p); }
...
class allocator_multialloc_chain_node_deallocator {
 ...
 public:
  ...
  chain_builder get_chain_builder() { return chain_builder(a_, c_); }
