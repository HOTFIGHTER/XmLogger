...
class vec_iterator {
 public:
  ...
  typedef typename if_c
      <IsConst, typename mars_boost::intrusive::pointer_traits<Pointer>::template
 ...
 public:
  ...
  vec_iterator(vec_iterator<Pointer, false> const &other)
  BOOST_NOEXCEPT_OR_NOTHROW
      :  m_ptr(other.get_ptr()) {}
  ...
  friend vec_iterator operator+(const vec_iterator &x, difference_type off)
  BOOST_NOEXCEPT_OR_NOTHROW
  { return vec_iterator(x.m_ptr + off); }

  friend vec_iterator operator+(difference_type off, vec_iterator right)
  BOOST_NOEXCEPT_OR_NOTHROW
  {
    right.m_ptr += off;
    return right;
  ...
  friend vec_iterator operator-(vec_iterator left, difference_type off)
  BOOST_NOEXCEPT_OR_NOTHROW
  {
    left.m_ptr -= off;
    return left;
  ...
  friend difference_type operator-(const vec_iterator &left, const vec_iterator &right)
  BOOST_NOEXCEPT_OR_NOTHROW
  { return left.m_ptr - right.m_ptr; }

  //Comparison operators
  friend bool operator==(const vec_iterator &l, const vec_iterator &r)
  BOOST_NOEXCEPT_OR_NOTHROW
...
struct vector_insert_ordered_cursor {
  ...
  size_type get_pos() const { return *last_position_it; }

  reference get_val() { return *last_value_it; }
...
struct vector_merge_cursor {
  ...
  size_type get_pos() const { return static_cast<size_type>((m_pcur + 1) - m_pbeg); }

  reference get_val() { return *m_valueit; }
...
template<class Pointer, bool IsConst>
const Pointer &vector_iterator_get_ptr(const container_detail::vec_iterator<Pointer, IsConst> &it)
BOOST_NOEXCEPT_OR_NOTHROW {
return   it.
get_ptr();
}
...
template<class Pointer, bool IsConst>
Pointer &get_ptr(container_detail::vec_iterator <Pointer, IsConst> &it)
BOOST_NOEXCEPT_OR_NOTHROW {
return  it.
get_ptr();
}
...
struct vector_alloc_holder
 ...
 public:
  ...
  static bool is_propagable_from(const allocator_type &from_alloc,
                                 pointer p,
                                 const allocator_type &to_alloc,
  ...
  static bool are_swap_propagable(const allocator_type &l_a,
                                  pointer l_p,
                                  const allocator_type &r_a,
                                  pointer r_p,
                                  bool const propagate_allocator) {
    (void) propagate_allocator;
    (void) l_p;
  ...
  explicit vector_alloc_holder (BOOST_FWD_REF(AllocConvertible)
  a)
  BOOST_NOEXCEPT_OR_NOTHROW
  ...
  //Constructor, does not throw
  template<class AllocConvertible>
  vector_alloc_holder(uninitialized_size_t,
                      BOOST_FWD_REF(AllocConvertible)
                      a,
                      size_type initial_size)
      : Allocator(mars_boost::forward<AllocConvertible>(a)), m_start(),
        m_size(initial_size)  //Size is initialized here so vector should only call uninitialized_xxx after this
      , m_capacity() {
    if (initial_size) {
      pointer reuse = 0;
      m_start =
          this->allocation_command(allocate_new, initial_size, m_capacity = initial_size, reuse);
    }
  }
  ...
  vector_alloc_holder (BOOST_RV_REF(vector_alloc_holder)
  holder)
  BOOST_NOEXCEPT_OR_NOTHROW
  ...
  vector_alloc_holder(pointer p, size_type capacity, BOOST_RV_REF(vector_alloc_holder) holder)
      : Allocator(BOOST_MOVE_BASE(Allocator, holder)), m_start(p), m_size(holder.m_size),
        m_capacity(capacity) {
  ...
  vector_alloc_holder(pointer p, size_type n, BOOST_FWD_REF(AllocFwd) a)
      : Allocator(::mars_boost::forward<AllocFwd>(a)), m_start(p), m_size(), m_capacity(n) {}
  ...
  pointer allocation_command(mars_boost::container::allocation_type command,
    ...
    return this->priv_allocation_command(alloc_version(),
                                         command,
                                         limit_size,
  ...
  void swap_resources(vector_alloc_holder &x)
  BOOST_NOEXCEPT_OR_NOTHROW
  {
  ...
  void steal_resources(vector_alloc_holder &x)
  BOOST_NOEXCEPT_OR_NOTHROW
  {
  ...
  BOOST_NOEXCEPT_OR_NOTHROW { return m_start; }
  const size_type &capacity() const
  BOOST_NOEXCEPT_OR_NOTHROW { return m_capacity; }
  void start(const pointer &p)
  BOOST_NOEXCEPT_OR_NOTHROW { m_start = p; }
  void capacity(const size_type &c)
  BOOST_NOEXCEPT_OR_NOTHROW { m_capacity = c; }
 ...
 private:
  ...
  pointer priv_allocation_command(version_1, mars_boost::container::allocation_type command,
    ...
    pointer const
        p = allocator_traits_type::allocate(this->alloc(), prefer_in_recvd_out_size, reuse);
    reuse = pointer();
    return p;
  }
...
struct vector_alloc_holder<Allocator, version_0>
 ...
 public:
  ...
  //Constructor, does not throw
  vector_alloc_holder(uninitialized_size_t, size_type initial_size)
      : Allocator(), m_size(initial_size)  //Size is initialized here...
  {
    //... and capacity here, so vector, must call uninitialized_xxx in the derived constructor
    this->priv_first_allocation(initial_size);
  }
        ...
        m_size(holder.m_size) //Size is initialized here so vector should only call uninitialized_xxx after this
  {
    ::mars_boost::container::uninitialized_move_alloc_n
        (this->alloc(),
         container_detail::to_raw_pointer(holder.start()),
         m_size,
         container_detail::to_raw_pointer(this->start()));
  }

  template<class OtherAllocator, class OtherAllocatorVersion>
  vector_alloc_holder(BOOST_RV_REF_BEG vector_alloc_holder
                      <OtherAllocator, OtherAllocatorVersion> BOOST_RV_REF_END
                      holder)
      : Allocator(),
  ...
  void deep_swap(vector_alloc_holder<OtherAllocator, OtherAllocatorVersion> &x) {
    if (this->m_size > OtherAllocator::internal_capacity
        || x.m_size > Allocator::internal_capacity) {
      throw_bad_alloc();
    }
    this->priv_deep_swap(x);
  }
  ...
  void
  steal_resources(vector_alloc_holder &) {  //Containers with version 0 allocators can't be moved without moving elements one by one
    throw_bad_alloc();
  }
  ...
  bool try_expand_fwd(size_type at_least) { return !at_least; }

  pointer start() const
  BOOST_NOEXCEPT_OR_NOTHROW { return Allocator::internal_storage(); }
  size_type capacity() const
  BOOST_NOEXCEPT_OR_NOTHROW { return Allocator::internal_capacity; }
  size_type m_size;
 ...
 private:
  ...
  void priv_deep_swap(vector_alloc_holder<OtherAllocator, OtherAllocatorVersion> &x) {
    ...
    if (this->m_size < x.m_size) {
      mars_boost::container::deep_swap_alloc_n<MaxTmpStorage>(this->alloc(),
                                                              first_this,
                                                              this->m_size,
                                                              first_x,
                                                              x.m_size);
...
class vector {
  ...
  struct value_less {
    ...
    bool operator()(const value_type &a, const value_type &b) const { return a < b; }
  };
 ...
 protected:
  static bool is_propagable_from(const Allocator &from_alloc,
                                 pointer_impl p,
                                 const Allocator &to_alloc,
                                 bool const propagate_allocator) {
    return alloc_holder_t::is_propagable_from(from_alloc,
                                              p,
                                              to_alloc,
                                              propagate_allocator);
  }

  static bool are_swap_propagable(const Allocator &l_a,
                                  pointer_impl l_p,
                                  const Allocator &r_a,
                                  pointer_impl r_p,
                                  bool const propagate_allocator) {
    return alloc_holder_t::are_swap_propagable(l_a,
                                               l_p,
                                               r_a,
                                               r_p,
                                               propagate_allocator);
 ...
 public:
  ...
  typedef typename ::mars_boost::container::allocator_traits<Allocator>::const_pointer
      const_pointer;
  typedef typename ::mars_boost::container::allocator_traits<Allocator>::reference reference;
  typedef typename ::mars_boost::container::allocator_traits<Allocator>::const_reference
      const_reference;
  typedef typename ::mars_boost::container::allocator_traits<Allocator>::size_type size_type;
  typedef typename ::mars_boost::container::allocator_traits<Allocator>::difference_type
      difference_type;
  typedef Allocator allocator_type;
...
#else
  typedef BOOST_CONTAINER_IMPDEF(iterator_impl)
  iterator;
  typedef BOOST_CONTAINER_IMPDEF(const_iterator_impl)
  const_iterator;
#endif
  typedef BOOST_CONTAINER_IMPDEF(mars_boost
  ::container::reverse_iterator <iterator>)
  reverse_iterator;
  typedef BOOST_CONTAINER_IMPDEF(mars_boost
  ::container::reverse_iterator <const_iterator>)
 ...
 protected:
  ...
  void steal_resources(vector &x) { return this->m_holder.steal_resources(x.m_holder); }
 ...
 public:
  ...
  vector(InIt first, InIt last)
      :  m_holder() { this->assign(first, last); }
  ...
  vector(InIt first, InIt last, const allocator_type &a)
      :  m_holder(a) { this->assign(first, last); }
  ...
  vector(const vector &x)
    ...
    ::mars_boost::container::uninitialized_copy_alloc_n
        (this->m_holder.alloc(), x.priv_raw_begin(), x.size(), this->priv_raw_begin());
  }
  ...
  x)
  BOOST_NOEXCEPT_OR_NOTHROW
      :  m_holder(mars_boost::move(x.m_holder)) { BOOST_STATIC_ASSERT((!allocator_traits_type::is_partially_propagable::value)); }
  ...
  vector(BOOST_RV_REF_BEG vector
         <T, OtherAllocator> BOOST_RV_REF_END
         x,
  ...
  vector(BOOST_RV_REF(vector) x, const allocator_type &a)
      : m_holder(container_detail::uninitialized_size,
                 a,
                 is_propagable_from(x.get_stored_allocator(), x.m_holder.start(), a, true) ? 0
                                                                                           : x.size()
  ) {
    if (is_propagable_from(x.get_stored_allocator(), x.m_holder.start(), a, true)) {
      ...
      ::mars_boost::container::uninitialized_move_alloc_n_source
          (this->m_holder.alloc(), x.priv_raw_begin(), n, this->priv_raw_begin());
    }
  }
  ...
  )
  {
    ...
    if (first == last) {
      ...
      const size_type
          n = static_cast<size_type>(end_pos - container_detail::iterator_to_raw_pointer(cur));
  ...
  )
  {
    ...
    const size_type
        input_sz = static_cast<size_type>(mars_boost::container::iterator_distance(first, last));
    ...
    if (input_sz > old_capacity) {  //If input range is too big, we need to reallocate
      ...
      pointer const ret(this->m_holder.allocation_command(allocate_new | expand_fwd,
                                                          input_sz,
                                                          real_cap = input_sz,
                                                          reuse));
  ...
  void assign(size_type n, const value_type &val) {
    this->assign(cvalue_iterator(val, n),
                 cvalue_iterator());
  }
  ...
  void resize(size_type new_size) { this->priv_resize(new_size, value_init); }
  ...
  //!
  //! <b>Note</b>: Non-standard extension
  void resize(size_type new_size, default_init_t) { this->priv_resize(new_size, default_init); }
  ...
  //!
  //! <b>Complexity</b>: Linear to the difference between size() and new_size.
  void resize(size_type new_size, const T &x) { this->priv_resize(new_size, x); }
  ...
  void shrink_to_fit() { this->priv_shrink_to_fit(alloc_version()); }
  ...
  template<class ...Args>
  void emplace_back(BOOST_FWD_REF(Args)...
  args)
  {
  ...
  //! <b>Note</b>: Non-standard extension.
  template<class ...Args>
  bool stable_emplace_back(BOOST_FWD_REF(Args)...
  args)
  {
    const bool is_room_enough =
        this->room_enough() || (alloc_version::value == 2 && this->m_holder.try_expand_fwd(1u));
  ...
  template<class ...Args>
  iterator emplace(const_iterator position, BOOST_FWD_REF(Args) ...
  args)
  {
    BOOST_ASSERT(this->priv_in_range_or_end(position));
    //Just call more general insert(pos, size, value) and return iterator
    typedef container_detail::insert_emplace_proxy<Allocator, T *, Args...> type;
    return this->priv_forward_range_insert(vector_iterator_get_ptr(position),
                                           1,
   ...
   void push_back(T &&x);
#else
  BOOST_MOVE_CONVERSION_AWARE_CATCH(push_back, T,
  void, priv_push_back)
#endif
  ...
  template<class FwdIt>
  iterator
  insert(const_iterator pos, FwdIt first, FwdIt last, typename container_detail::disable_if_or
      <void,
    ...
    return this->priv_forward_range_insert(vector_iterator_get_ptr(pos),
                                           mars_boost::container::iterator_distance(first, last),
                                           proxy);
  }
#endif
  ...
  iterator erase(const_iterator position) {
    ...
    T *const new_end_ptr =
        ::mars_boost::container::move(pos_ptr + 1, beg_ptr + this->m_holder.m_size, pos_ptr);
  ...
  iterator erase(const_iterator first, const_iterator last) {
    ...
    if (first != last) {
      ...
      T *const ptr = container_detail::to_raw_pointer(mars_boost::container::move(last_ptr,
                                                                                  old_end_ptr,
                                                                                  first_ptr));
  ...
  {
    this->priv_swap(x,
                    container_detail::bool_<container_detail::is_version<Allocator, 0>::value>());
  }
  ...
  //! <b>Note</b>: Non-standard extension to support static_vector
  template<class OtherAllocator>
  void swap(vector<T, OtherAllocator> &x, typename container_detail::enable_if_and
      <void,
       container_detail::is_version<OtherAllocator, 0>,
       container_detail::is_different<OtherAllocator, allocator_type>
      >::type * = 0
  ...
  friend bool operator==(const vector &x, const vector &y) {
    return x.size() == y.size()
        && ::mars_boost::container::algo_equal(x.begin(), x.end(), y.begin());
  }
  ...
  bool stable_reserve(size_type new_cap) {
    ...
    return cp >= new_cap
        || (alloc_version::value == 2 && this->m_holder.try_expand_fwd(new_cap - cp));
  }

  //Absolutely experimental. This function might change, disappear or simply crash!
  template<class BiDirPosConstIt, class BiDirValueIt>
  void insert_ordered_at(const size_type element_count,
                         BiDirPosConstIt last_position_it,
                         BiDirValueIt last_value_it) {
    typedef container_detail::vector_insert_ordered_cursor<BiDirPosConstIt, BiDirValueIt>
        inserter_t;
    return this->priv_insert_ordered_at(element_count, inserter_t(last_position_it, last_value_it));
  }

  template<class BidirIt>
  void merge(BidirIt first, BidirIt last) { this->merge(first, last, value_less()); }

  template<class BidirIt, class Compare>
  void merge(BidirIt first,
             BidirIt last,
             Compare comp) { this->priv_merge(container_detail::false_type(), first, last, comp); }

  template<class BidirIt>
  void merge_unique(BidirIt first, BidirIt last) {
    this->priv_merge(container_detail::true_type(),
                     first,
                     last,
                     value_less());
  }
  ...
  template<class BidirIt, class Compare>
  void merge_unique(BidirIt first,
                    BidirIt last,
                    Compare comp) {
    this->priv_merge(container_detail::true_type(),
                     first,
 ...
 private:
  ...
  void priv_insert_ordered_at(const size_type element_count, PositionValue position_value) {
    ...
    while (insertions_left) {
      ...
      size_type new_hole_size = (pos != prev_pos)
                                ? priv_insert_ordered_at_shift_range(pos,
                                                                     prev_pos,
                                                                     this->size(),
                                                                     insertions_left)
                                : old_hole_size;
      if (new_hole_size) {
        ...
        allocator_traits_type::construct(this->m_holder.alloc(),
                                         begin_ptr + pos + insertions_left - 1,
                                         position_value.get_val());
        if (--new_hole_size) {
  ...
  void priv_merge(UniqueBool, BidirIt first, BidirIt last, Compare comp) {
    size_type const
        n = static_cast<size_type>(mars_boost::container::iterator_distance(first, last));
      ...
      if (free_c < n) {
        ...
        while (remaining) {
          ...
          std::size_t index_capacity =
              (aligned_addr >= capaddr) ? 0u : (capaddr - addr) / sizeof(size_type);
          ...
          this->priv_insert_ordered_range(UniqueBool(),
                                          index_capacity,
                                          first,
                                          limit,
                                          indexes,
      ...
      while (1) {
        ...
        if (comp(*first, *pcur)) {
          *position = static_cast<size_type>(pcur - pbeg);
          BOOST_ASSERT((position == positions) || (*(position - 1) == size_type(-1))
                           || (*(position - 1) <= *position));
          ++position;
          ++added_in_middle;
          --remaining;
  ...
  void priv_merge_in_new_buffer
    ...
    while (1) {
      if (!n) {
        ::mars_boost::container::uninitialized_move_alloc(this->m_holder.alloc(),
                                                          pbeg,
                                                          pend,
                                                          d_first);
        break;
      } else if (pbeg == pend) {
        ::mars_boost::container::uninitialized_move_alloc_n(this->m_holder.alloc(),
                                                            first,
                                                            n,
                                                            d_first);
        break;
  ...
  bool room_enough() const { return this->m_holder.m_size < this->m_holder.capacity(); }

  pointer back_ptr() const { return this->m_holder.start() + this->m_holder.m_size; }
  ...
  template<class OtherAllocator>
  void priv_move_assign(BOOST_RV_REF_BEG vector
                        <T, OtherAllocator> BOOST_RV_REF_END
                        x,
                        typename container_detail::enable_if_c
                            <container_detail::is_version<OtherAllocator, 0>::value>::type * = 0) {
    ...
    mars_boost::container::move_assign_range_alloc_n(this->m_holder.alloc(),
                                                     other_start,
                                                     other_sz,
                                                     this_start,
                                                     this_sz);
  ...
  void priv_move_assign(BOOST_RV_REF_BEG vector
    ...
    const bool
        propagate_alloc = allocator_traits_type::propagate_on_container_move_assignment::value;

    const bool is_propagable_from_x =
        is_propagable_from(x_alloc, x.m_holder.start(), this_alloc, propagate_alloc);
    const bool is_propagable_from_t =
        is_propagable_from(this_alloc, m_holder.start(), x_alloc, propagate_alloc);
  ...
  template<class OtherAllocator>
  void priv_copy_assign(const vector<T, OtherAllocator> &x, typename container_detail::enable_if_c
      <container_detail::is_version<OtherAllocator, 0>::value>::type * = 0) {
    if (!container_detail::is_same<OtherAllocator, allocator_type>::value &&
        this->capacity() < x.size()) {
      throw_bad_alloc();
    ...
    const size_type this_sz = m_holder.m_size;
    const size_type other_sz = static_cast<size_type>(x.m_holder.m_size);
    mars_boost::container::copy_assign_range_alloc_n(this->m_holder.alloc(),
                                                     other_start,
                                                     other_sz,
                                                     this_start,
                                                     this_sz);
  ...
  void priv_swap(Vector &x, container_detail::false_type)  //version_N
  {
    ...
    if (are_swap_propagable(this->get_stored_allocator(),
                            this->m_holder.start(),
      ...
      sml.insert(sml.cend(),
                 mars_boost::make_move_iterator(container_detail::iterator_to_raw_pointer(big.nth(
                     common_elements))),
    ...
    container_detail::swap_alloc(this->m_holder.alloc(),
                                 x.m_holder.alloc(),
                                 container_detail::bool_<propagate_alloc>());
  }

  void priv_reserve_no_capacity(size_type, version_0) { throw_bad_alloc(); }

  container_detail::insert_range_proxy<Allocator, mars_boost::move_iterator<T *>, T *>
  priv_dummy_empty_proxy() {
    return container_detail::insert_range_proxy<Allocator, mars_boost::move_iterator<T *>, T *>
        (::mars_boost::make_move_iterator((T * )
    0));
  }
  ...
  void priv_reserve_no_capacity(size_type new_cap, version_2) {
    ...
    size_type real_cap = 0;
    pointer reuse = 0;
    pointer const ret(this->m_holder.allocation_command(allocate_new | expand_fwd | expand_bwd,
                                                        new_cap,
                                                        real_cap = new_cap,
                                                        reuse));
  ...
  void priv_destroy_last(const bool moved = false)
  BOOST_NOEXCEPT_OR_NOTHROW
  {
    (void) moved;
  ...
  void priv_destroy_last_n(const size_type n)
  BOOST_NOEXCEPT_OR_NOTHROW
  {
  ...
  void priv_uninitialized_construct_at_end(InpIt first, InpIt last) {
    ...
    T *const new_end_pos = mars_boost::container::uninitialized_copy_alloc(this->m_holder.alloc(),
                                                                           first,
                                                                           last,
                                                                           old_end_pos);
  ...
  iterator priv_insert(const const_iterator &p, BOOST_FWD_REF(U) x) {
    ...
    return this->priv_forward_range_insert
        (vector_iterator_get_ptr(p),
         1,
         container_detail::get_insert_value_proxy<T *, Allocator>(::mars_boost::forward<U>(x)));
  }
  ...
  container_detail::insert_move_proxy<Allocator, T *> priv_single_insert_proxy(BOOST_RV_REF(T)
                                                                               x) {
    return container_detail::insert_move_proxy<Allocator,
                                               T *>(x);
  }
  ...
  void priv_push_back(BOOST_FWD_REF(U) u) {
    if (BOOST_LIKELY(this->room_enough())) {
      ...
      this->priv_forward_range_insert_no_capacity
          (this->back_ptr(),
           1,
           this->priv_single_insert_proxy(::mars_boost::forward<U>(u)),
           alloc_version());
    }
  }

  container_detail::insert_n_copies_proxy<Allocator, T *> priv_resize_proxy(const T &x) {
    return container_detail::insert_n_copies_proxy<Allocator,
                                                   T *>(x);
  }

  container_detail::insert_default_initialized_n_proxy<Allocator, T *>
  priv_resize_proxy(default_init_t) {
    return container_detail::insert_default_initialized_n_proxy<Allocator,
                                                                T *>();
  }

  container_detail::insert_value_initialized_n_proxy<Allocator, T *>
  priv_resize_proxy(value_init_t) {
    return container_detail::insert_value_initialized_n_proxy<Allocator,
                                                              T *>();
  }
  ...
  void priv_shrink_to_fit(version_1) {
    ...
    if (cp) {
      ...
      if (!sz) {
        ...
        //Pass the hint so that allocators can take advantage of this.
        pointer const
            p = allocator_traits_type::allocate(this->m_holder.alloc(), sz, this->m_holder.m_start);
  ...
  void priv_shrink_to_fit(version_2)
  BOOST_NOEXCEPT_OR_NOTHROW
  {
    const size_type cp = this->m_holder.capacity();
    if (cp) {
 ...
 private:
  T *priv_raw_begin() const { return container_detail::to_raw_pointer(m_holder.start()); }

  T *priv_raw_end() const { return this->priv_raw_begin() + this->m_holder.m_size; }

  template<class InsertionProxy>
  void priv_forward_range_insert_at_end_expand_forward(const size_type n,
  ...
  template<class InsertionProxy>
  void priv_forward_range_insert_expand_forward(T *const pos,
                                                const size_type n,
                                                InsertionProxy insert_range_proxy) {
  ...
  void priv_forward_range_insert_new_allocation
    ...
    typename value_traits::ArrayDeallocator
        new_buffer_deallocator(new_start, this->m_holder.alloc(), new_cap);
    typename value_traits::ArrayDestructor
        new_values_destroyer(new_start, this->m_holder.alloc(), 0u);
    ...
    if (old_buffer) {
      ...
      if (!value_traits::trivial_dctr_after_move)
        mars_boost::container::destroy_alloc_n(this->get_stored_allocator(),
                                               old_buffer,
                                               this->m_holder.m_size);
  ...
  void priv_forward_range_insert_expand_backwards
    ...
    typename value_traits::ArrayDestructor
        old_values_destroyer(old_start, this->m_holder.alloc(), old_size);
    ...
    if (s_before >= before_plus_new) {
      ...
      if (do_after) {
        ...
        if (elemsafter >= n_after) {
          ...
          insert_range_proxy.uninitialized_copy_n_and_update(this->m_holder.alloc(),
                                                             old_finish,
                                                             mid_last_dist);
          this->m_holder.m_size += n_after;
