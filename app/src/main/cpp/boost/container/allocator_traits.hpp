...
BOOST_INTRUSIVE_HAS_STATIC_MEMBER_FUNC_SIGNATURE(has_select_on_container_copy_construction,
...
template<class Allocator>
struct is_not_std_allocator { static const bool value = !is_std_allocator<Allocator>::value; };
...
struct allocator_traits {
...
#else
  //pointer
  typedef BOOST_INTRUSIVE_OBTAIN_TYPE_WITH_DEFAULT(mars_boost
  ::container::container_detail::, Allocator,
  pointer, value_type*)
  pointer;
  //const_pointer
  typedef BOOST_INTRUSIVE_OBTAIN_TYPE_WITH_EVAL_DEFAULT(mars_boost
  ::container::container_detail::, Allocator,
  ...
  //C++11
  template<typename T> using rebind_alloc  = typename mars_boost::intrusive::pointer_rebind<
      Allocator,
      T>::type;
...
#undef BOOST_CONTAINER_ALLOCATOR_TRAITS_REBIND_ALLOC
  ...
  template<class T>
  struct portable_rebind_alloc {
    typedef typename mars_boost::intrusive::pointer_rebind<Allocator,
                                                           T>::type type;
  };
  ...
  //!
  //! <b>Throws</b>: Nothing
  static void deallocate(Allocator &a, pointer p, size_type n) { a.deallocate(p, n); }
  ...
  BOOST_NOEXCEPT_OR_NOTHROW
  {
    const bool value = allocator_traits_detail::has_max_size < Allocator,
        size_type(Allocator::*)() const>::value;
    container_detail::bool_ <value> flag;
  ...
  typename container_detail::if_c
      <allocator_traits_detail::has_select_on_container_copy_construction<
  Allocator BOOST_MOVE_I
  ...
  static void construct(Allocator &a, T *p, BOOST_FWD_REF(Args)...
  args)
  {
  ...
  //! <code>false</code>.
  static bool storage_is_unpropagable(const Allocator &a, pointer p)
  BOOST_NOEXCEPT_OR_NOTHROW
  {
  ...
  //! <code>a == b</code>.
  static bool equal(const Allocator &a, const Allocator &b)
  BOOST_NOEXCEPT_OR_NOTHROW
  {
...
#if !defined(BOOST_CONTAINER_DOXYGEN_INVOKED)
 private:
  static pointer priv_allocate(container_detail::true_type,
                               Allocator &a,
                               size_type n,
                               const_void_pointer p) { return a.allocate(n, p); }

  static pointer priv_allocate(container_detail::false_type,
                               Allocator &a,
                               size_type n,
                               const_void_pointer) { return a.allocate(n); }

  template<class T>
  static void priv_destroy(container_detail::true_type, Allocator &a, T *p)
  BOOST_NOEXCEPT_OR_NOTHROW
  { a.destroy(p); }

  template<class T>
  static void priv_destroy(container_detail::false_type, Allocator &, T *p)
  BOOST_NOEXCEPT_OR_NOTHROW
  {
    p->~T();
    (void) p;
  ...
  static size_type priv_max_size(container_detail::true_type, const Allocator &a)
  BOOST_NOEXCEPT_OR_NOTHROW
  { return a.max_size(); }

  static size_type priv_max_size(container_detail::false_type, const Allocator &)
  BOOST_NOEXCEPT_OR_NOTHROW
  { return size_type(-1) / sizeof(value_type); }

  static Allocator priv_select_on_container_copy_construction(container_detail::true_type,
                                                              const Allocator &a) { return a.select_on_container_copy_construction(); }

  static const Allocator &
  priv_select_on_container_copy_construction(container_detail::false_type, const Allocator &a)
  BOOST_NOEXCEPT_OR_NOTHROW
  { return a; }
  ...
  template<class T, class ...Args>
  static void
  priv_construct(container_detail::true_type, Allocator &a, T *p, BOOST_FWD_REF(Args) ...
  args)
  { a.construct(p, ::mars_boost::forward<Args>(args)...); }

  template<class T, class ...Args>
  static void
  priv_construct(container_detail::false_type, Allocator &, T *p, BOOST_FWD_REF(Args) ...
  args)
...
#undef BOOST_CONTAINER_ALLOCATOR_TRAITS_PRIV_CONSTRUCT_IMPL
  ...
  template<class T>
  static void priv_construct(container_detail::false_type,
                             Allocator &,
                             T *p,
                             const ::mars_boost::container::default_init_t &) {
    ::new((void *) p,
          boost_container_new_t()) T;
  }

  static bool priv_storage_is_unpropagable(container_detail::true_type,
                                           const Allocator &a,
                                           pointer p) { return a.storage_is_unpropagable(p); }

  static bool priv_storage_is_unpropagable(container_detail::false_type,
                                           const Allocator &,
                                           pointer) { return false; }

  static bool
  priv_equal(container_detail::true_type, const Allocator &, const Allocator &) { return true; }
