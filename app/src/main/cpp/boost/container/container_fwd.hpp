...
template<class Key, class T, class Compare  = std::less <Key>, class Allocator = new_allocator<std::pair<
    const Key,
    T> >, class Options = tree_assoc_defaults>
class map;
...
template<class Key, class T, class Compare  = std::less <Key>, class Allocator = new_allocator<
    std::pair < Key, T> > >
class flat_map;
...
template<class CharT, class Traits = std::char_traits <CharT>, class Allocator  = new_allocator<
    CharT> >
class basic_string;
...
template
    <class T, std::size_t NodesPerBlock = NodeAlloc_nodes_per_block, std::size_t Version = 2>
