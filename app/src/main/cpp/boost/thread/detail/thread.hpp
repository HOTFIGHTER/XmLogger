...
class BOOST_THREAD_DECL thread {
...
#else
  template<typename F>
  static inline detail::thread_data_ptr make_thread_info(F f, typename disable_if_c<
 ...
 public:
  ...
  thread(F f,
         A1 a1,
         typename disable_if<boost::thread_detail::is_convertible<F &, thread_attributes>,
                             dummy *>::type= 0):
  ...
  thread(F f, A1 a1, A2 a2, A3 a3, A4 a4, A5 a5):
      thread_info(make_thread_info(mars_boost::bind(mars_boost::type<void>(),
                                                    f,
                                                    a1,
                                                    a2,
                                                    a3,
...
void thread::join() {
  if (this_thread::get_id() == get_id())
    mars_boost::throw_exception(thread_resource_error(static_cast<int>(system::errc::resource_deadlock_would_occur),
...
void at_thread_exit(F f) {
  detail::thread_exit_function_base
      *const thread_exit_func = detail::heap_new<detail::thread_exit_function<F> >(f);
