...
class nullary_function<void()> {
  ...
  struct impl_type: impl_base {
    ...
    {}
#endif
    impl_type (BOOST_THREAD_RV_REF(F)
    f_)
    :
 ...
 public:
...
#endif
  template<typename F>
  nullary_function (BOOST_THREAD_RV_REF(F)
  f):
  impl(new
  impl_type<typename decay<F>::type>(thread_detail::decay_copy(mars_boost::forward<F>(f)))
  )
  {}
  ...
  nullary_function(nullary_function const &other)
  BOOST_NOEXCEPT :
  impl(other.impl) {
  }
  nullary_function(BOOST_THREAD_RV_REF(nullary_function) other)
  BOOST_NOEXCEPT :
  ...
  nullary_function &operator=(BOOST_THREAD_COPY_ASSIGN_REF(nullary_function) other)
  BOOST_NOEXCEPT
  {
    impl = other.impl;
    return *this;
  }
  nullary_function &operator=(BOOST_THREAD_RV_REF(nullary_function) other)
  BOOST_NOEXCEPT
  {
#if defined BOOST_NO_CXX11_SMART_PTR
    impl=BOOST_THREAD_RV(other).impl;
...
class nullary_function<R()> {
 ...
 public:
  ...
  R operator()() { if (impl) return impl->call(); else return R(); }
