...
//#endif
  template<typename F>
  function_wrapper (BOOST_THREAD_RV_REF(F)
  f):
  impl(new
  impl_type<F>(mars_boost::forward<F>(f))
  )
  {}
  function_wrapper(BOOST_THREAD_RV_REF(function_wrapper) other)
  BOOST_NOEXCEPT :
  impl(other.impl) {
    other.impl = 0;
  }
  ...
  function_wrapper &operator=(BOOST_THREAD_RV_REF(function_wrapper) other)
  BOOST_NOEXCEPT
  {
    impl = other.impl;
    other.impl = 0;
