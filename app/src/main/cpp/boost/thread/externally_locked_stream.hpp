...
class stream_guard {
 ...
 public:
  ...
  stream_guard (BOOST_THREAD_RV_REF(stream_guard)
  rhs)
  BOOST_NOEXCEPT
      : mtx_(rhs.mtx_) {
    rhs.mtx_ = 0;
  ...
  bool owns_lock(const mutex_type *l) const
  BOOST_NOEXCEPT
  {
    return l == mtx_->mutex();
  }
...
inline const stream_guard<Stream, RecursiveMutex> &
...
template<typename Stream, typename RecursiveMutex, typename T>
inline stream_guard<Stream, RecursiveMutex>
