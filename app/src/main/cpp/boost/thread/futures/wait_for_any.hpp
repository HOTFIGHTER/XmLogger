  ...
  struct all_futures_lock {
    ...
    all_futures_lock(std::vector <registered_waiter> &waiters) :
      ...
      for (count_type_portable i = 0; i < count; ++i) {
        locks[i] =
            BOOST_THREAD_MAKE_RV_REF(mars_boost::unique_lock<boost::mutex>(waiters[i].future_->mutex()));
      }
    }
...
typename mars_boost::disable_if<is_future_type < Iterator> , Iterator>
::type wait_for_any(Iterator begin, Iterator end) {
