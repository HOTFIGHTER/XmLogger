 ...
 private:
  ...
  void release_waiters(state_data old_state) {
    ...
    if (old_state.shared_waiting || old_state.exclusive_waiting) {
      BOOST_VERIFY(detail::win32::ReleaseSemaphore(semaphores[unlock_sem],
                                                   old_state.shared_waiting
                                                       + (old_state.exclusive_waiting ? 1 : 0),
                                                   0) != 0);
    }
  }
 ...
 public:
  ...
  void lock_upgrade() {
    for (;;) {
      ...
      BOOST_VERIFY(!detail::win32::WaitForSingleObjectEx(semaphores[unlock_sem],
  ...
  void unlock_upgrade_and_lock() {
    ...
    for (;;) {
      ...
      if (current_state == old_state) {
        if (!last_reader) {
          BOOST_VERIFY(!detail::win32::WaitForSingleObjectEx(upgrade_sem,
                                                             detail::win32::infinite,
                                                             0));
        }
        break;
