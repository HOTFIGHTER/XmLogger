...
class const_strict_lock_ptr {
 ...
 public:
   ...
   */
  const_strict_lock_ptr (BOOST_THREAD_RV_REF(const_strict_lock_ptr)
  other)
  BOOST_NOEXCEPT
      : lk_(mars_boost::move(BOOST_THREAD_RV(other).lk_)), value_(BOOST_THREAD_RV(other)
  .value_)
  {
  }
...
class strict_lock_ptr: public const_strict_lock_ptr<T, Lockable> {
 ...
 public:
   ...
   * @effects takes ownership of the mutex owned by @c other, stores a reference to the mutex and the value type of @c other.
   */
  strict_lock_ptr (BOOST_THREAD_RV_REF(strict_lock_ptr)
  other)
  :
  base_type (mars_boost::move(
...
class const_unique_lock_ptr: public unique_lock<Lockable> {
 ...
 public:
   ...
   */
  const_unique_lock_ptr(T const &val, Lockable &mtx, adopt_lock_t)
  BOOST_NOEXCEPT
   ...
   */
  const_unique_lock_ptr(T const &val, Lockable &mtx, defer_lock_t)
  BOOST_NOEXCEPT
   ...
   */
  const_unique_lock_ptr(T const &val, Lockable &mtx, try_to_lock_t)
  BOOST_NOEXCEPT
   ...
   * @effects takes ownership of the mutex owned by @c other, stores a reference to the mutex and the value type of @c other.
   */
  const_unique_lock_ptr (BOOST_THREAD_RV_REF(const_unique_lock_ptr)
  other)
  BOOST_NOEXCEPT
      : base_type(mars_boost::move(static_cast<base_type &>(other))), value_(BOOST_THREAD_RV(other)
  .value_)
  {
  }
...
class unique_lock_ptr: public const_unique_lock_ptr<T, Lockable> {
 ...
 public:
   ...
   * @effects takes ownership of the mutex owned by @c other, stores a reference to the mutex and the value type of @c other.
   */
  unique_lock_ptr (BOOST_THREAD_RV_REF(unique_lock_ptr)
  other)
  BOOST_NOEXCEPT
...
class synchronized_value {
  ...
  template<typename ...SV>
  friend std::tuple<typename synchronized_value_strict_lock_ptr<SV>::type ...>
  synchronize(SV &...sv);
#else
 ...
 public:
   ...
   */
  synchronized_value (BOOST_THREAD_RV_REF(T)
  other)
  ...
  unique_lock_ptr<T, Lockable> defer_synchronize()
  BOOST_NOEXCEPT
  {
    return BOOST_THREAD_MAKE_RV_REF((unique_lock_ptr<T, Lockable>(value_, mtx_, defer_lock)));
  }
  const_unique_lock_ptr<T, Lockable> defer_synchronize() const
  BOOST_NOEXCEPT
  {
    return BOOST_THREAD_MAKE_RV_REF((const_unique_lock_ptr<T, Lockable>(value_, mtx_, defer_lock)));
  }
  unique_lock_ptr<T, Lockable> try_to_synchronize()
  BOOST_NOEXCEPT
  {
    return BOOST_THREAD_MAKE_RV_REF((unique_lock_ptr<T, Lockable>(value_, mtx_, try_to_lock)));
  }
  const_unique_lock_ptr<T, Lockable> try_to_synchronize() const
  BOOST_NOEXCEPT
  {
    return BOOST_THREAD_MAKE_RV_REF((const_unique_lock_ptr<T, Lockable>(value_,
                                                                        mtx_,
                                                                        try_to_lock)));
  }
  unique_lock_ptr<T, Lockable> adopt_synchronize()
  BOOST_NOEXCEPT
  {
    return BOOST_THREAD_MAKE_RV_REF((unique_lock_ptr<T, Lockable>(value_, mtx_, adopt_lock)));
  }
  const_unique_lock_ptr<T, Lockable> adopt_synchronize() const
  BOOST_NOEXCEPT
  {
...
std::tuple<typename synchronized_value_strict_lock_ptr<SV>::type ...> synchronize(SV &...sv) {
  ...
  return t_type(typename synchronized_value_strict_lock_ptr<SV>::type(sv.value_,
                                                                      sv.mtx_,
                                                                      adopt_lock) ...);
}
#else
