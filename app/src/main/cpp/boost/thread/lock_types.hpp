...
class unique_lock {
 ...
 BOOST_THREAD_MOVABLE_ONLY(unique_lock)
  ...
  void lock() {
    if (m == 0) {
      mars_boost::throw_exception(
          mars_boost::lock_error(static_cast<int>(system::errc::operation_not_permitted),
                                 "boost unique_lock has no mutex"));
    }
    if (owns_lock()) {
      mars_boost::throw_exception(
          mars_boost::lock_error(static_cast<int>(system::errc::resource_deadlock_would_occur),
  ...
  bool timed_lock(TimeDuration const &relative_time) {
    if (m == 0) {
      mars_boost::throw_exception(mars_boost::lock_error(static_cast<int>(system::errc::operation_not_permitted),
                                                         "boost unique_lock has no mutex"));
    }
    if (owns_lock()) {
      mars_boost::throw_exception(mars_boost::lock_error(static_cast<int>(system::errc::resource_deadlock_would_occur),
                                                         "boost unique_lock owns already the mutex"));
    }
...
class shared_lock {
 ...
 BOOST_THREAD_MOVABLE_ONLY(shared_lock)
  ...
  shared_lock &
  operator=(BOOST_THREAD_RV_REF_BEG upgrade_lock<Mutex> BOOST_THREAD_RV_REF_END other) {
    shared_lock temp(::boost::move(other));
    swap(temp);
    return *this;
  }
...
#ifndef BOOST_THREAD_PROVIDES_EXPLICIT_LOCK_CONVERSION
  upgrade_lock &
  operator=(BOOST_THREAD_RV_REF_BEG unique_lock<Mutex> BOOST_THREAD_RV_REF_END other) {
    upgrade_lock temp(::boost::move(other));
    swap(temp);
    return *this;
  }
...
class upgrade_to_unique_lock {
 ...
 BOOST_THREAD_MOVABLE_ONLY(upgrade_to_unique_lock)
  ...
  //std-2104 unique_lock move-assignment should not be noexcept
  upgrade_to_unique_lock &
  operator=(BOOST_THREAD_RV_REF_BEG upgrade_to_unique_lock<Mutex> BOOST_THREAD_RV_REF_END other) //BOOST_NOEXCEPT
  {
...
#endif
  try_lock_wrapper &
  operator=(BOOST_THREAD_RV_REF_BEG try_lock_wrapper<Mutex> BOOST_THREAD_RV_REF_END other) {
