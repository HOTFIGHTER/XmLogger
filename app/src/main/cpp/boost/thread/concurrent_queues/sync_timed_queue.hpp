...
struct scheduled_type {
  ...
  scheduled_type (BOOST_THREAD_RV_REF(T)
  pdata,
  time_point tp
  ) :
  data (mars_boost::move(pdata)), time(tp) {}
  ...
  scheduled_type(BOOST_THREAD_RV_REF(scheduled_type) other)
      : data(mars_boost::move(other.data)), time(other.time) {}
...
class sync_timed_queue
 ...
 public:
  ...
  template<class Duration>
  queue_op_status
  try_push(BOOST_THREAD_RV_REF(T) elem, chrono::time_point <clock, Duration> const &tp);
  template<class Rep, class Period>
 ...
 private:
  ...
  template<class Duration>
  queue_op_status pull_when_time_reached_until(unique_lock <mutex> &,
...
template<class Duration>
void
sync_timed_queue<T, Clock>::push(const T &elem, chrono::time_point <clock, Duration> const &tp) {
...
template<class T, class Clock>
template<class Duration>
void sync_timed_queue<T, Clock>::push(BOOST_THREAD_RV_REF(T)
                                      elem,
                                      chrono::time_point <clock, Duration> const &tp) {
...
template<class T, class Clock>
template<class Duration>
queue_op_status sync_timed_queue<T, Clock>::try_push(const T &elem,
                                                     chrono::time_point <clock, Duration> const &tp) {
...
template<class Rep, class Period>
queue_op_status
sync_timed_queue<T, Clock>::try_push(const T &elem, chrono::duration <Rep, Period> const &dura) {
  return try_push(elem, clock::now() + dura);
}

template<class T, class Clock>
template<class Duration>
queue_op_status sync_timed_queue<T, Clock>::try_push(BOOST_THREAD_RV_REF(T)
                                                     elem,
                                                     chrono::time_point <clock, Duration> const &tp) {
...
template<class T, class Clock>
bool
sync_timed_queue<T, Clock>::wait_until_not_empty_time_reached_or_closed(unique_lock <mutex> &lk) {
  for (;;) {
...
template<class Duration>
queue_op_status
sync_timed_queue<T, Clock>::pull_when_time_reached_until(unique_lock <mutex> &lk,
                                                         chrono::time_point <clock, Duration> const &tp,
                                                         T &elem) {
  chrono::time_point <clock, Duration>
      tpmin = (tp < super::data_.top().time) ? tp : super::data_.top().time;
  while (time_not_reached(lk)) {
    super::throw_if_closed(lk);
