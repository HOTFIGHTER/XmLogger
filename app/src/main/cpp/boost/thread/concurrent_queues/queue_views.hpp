...
class queue_back_view {
 ...
 public:
  ...
  queue_back_view(Queue &q)
  BOOST_NOEXCEPT : queue(&q) {}
  ...
  queue_op_status
  try_push(BOOST_THREAD_RV_REF(value_type) x) { return queue->try_push(mars_boost::move(x)); }
  queue_op_status nonblocking_push(BOOST_THREAD_RV_REF(value_type)
                                   x) { return queue->nonblocking_push(mars_boost::move(x)); }
  queue_op_status
  wait_push(BOOST_THREAD_RV_REF(value_type) x) { return queue->wait_push(mars_boost::move(x)); }
};
...
class queue_front_view {
 ...
 public:
  ...
  queue_op_status try_push(BOOST_THREAD_RV_REF(value_type)
                           x) { return queue->try_push_front(forward<value_type>(x)); }
  queue_op_status nonblocking_push(BOOST_THREAD_RV_REF(value_type)
                                   x) { return queue->nonblocking_push_front(forward<value_type>(x)); }
