...
class sync_deque_base {
 ...
 protected:
  ...
  inline size_type size(lock_guard <mutex> &) const
  BOOST_NOEXCEPT
  {
    return data_.size();
  }
...
template<class ValueType, class Queue>
typename sync_deque_base<ValueType, Queue>::size_type
sync_deque_base<ValueType, Queue>::size() const {
  lock_guard <mutex> lk(mtx_);
  return size(lk);
}
...
template<class ValueType, class Queue>
queue_op_status
sync_deque_base<ValueType, Queue>::wait_until_not_empty_until(unique_lock <mutex> &lk,
                                                              time_point const &tp) {
  for (;;) {
