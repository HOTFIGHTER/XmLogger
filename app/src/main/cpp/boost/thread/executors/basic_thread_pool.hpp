...
class basic_thread_pool {
 ...
 public:
  ...
  template<class AtThreadEntry>
  basic_thread_pool(unsigned const thread_count,
                    BOOST_THREAD_FWD_REF(AtThreadEntry)
                    at_thread_entry) {
    try {
      threads.reserve(thread_count);
      for (unsigned i = 0; i < thread_count; ++i) {
        thread th(&basic_thread_pool::worker_thread3<AtThreadEntry>,
                  this,
                  mars_boost::forward<AtThreadEntry>(at_thread_entry));
        threads.push_back(thread_t(mars_boost::move(th)));
        //threads.push_back(thread_t(&basic_thread_pool::worker_thread, this)); // do not compile
      }
    }
