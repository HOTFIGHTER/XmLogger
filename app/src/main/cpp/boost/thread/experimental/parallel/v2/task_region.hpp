...
BOOST_THREAD_INLINE_NAMESPACE(v2)
    {
        class BOOST_SYMBOL_VISIBLE task_canceled_exception:
        ...
        class task_region_handle_gen
        {
          ...
          public:
          BOOST_DELETED_FUNCTION(task_region_handle_gen(
          const task_region_handle_gen&))
          BOOST_DELETED_FUNCTION(task_region_handle_gen & operator=(
          const task_region_handle_gen&))
...
#else
            group.push_back(async(detail::wrapped < task_region_handle_gen < Executor > ,
                                  F > (*this, forward<F>(f))));
#endif
#else
#if defined BOOST_THREAD_PROVIDES_EXECUTORS
        ...
        public task_region_handle_gen<default_executor>
        {
          ...
          BOOST_DELETED_FUNCTION(task_region_handle(
          const task_region_handle&))
          BOOST_DELETED_FUNCTION(task_region_handle & operator=(
          const task_region_handle&))
