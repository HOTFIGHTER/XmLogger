...
class mutex {
 ...
 BOOST_THREAD_NO_COPYABLE(mutex)
  ...
  void lock() {
    ...
    if (res) {
      mars_boost::throw_exception(lock_error(res,
...
class timed_mutex {
 ...
 BOOST_THREAD_NO_COPYABLE(timed_mutex)
  timed_mutex() {
    ...
    if (res) {
      mars_boost::throw_exception(thread_resource_error(res,
                                                        "boost:: timed_mutex constructor failed in pthread_mutex_init"));
    }
#ifndef BOOST_PTHREAD_HAS_TIMEDLOCK
    ...
    if (res2) {
      ...
      mars_boost::throw_exception(thread_resource_error(res2,
                                                        "boost:: timed_mutex constructor failed in pthread_cond_init"));
    }
    is_locked = false;
#endif
  }
