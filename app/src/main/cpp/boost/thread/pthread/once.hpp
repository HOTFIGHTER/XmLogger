...
BOOST_THREAD_DECL extern uintmax_atomic_t
once_global_epoch;
BOOST_THREAD_DECL extern pthread_mutex_t
once_epoch_mutex;
BOOST_THREAD_DECL extern pthread_cond_t
once_epoch_cv;
}
...
template<typename Function, class ...ArgTypes>
inline void
call_once(once_flag &flag, BOOST_THREAD_RV_REF(Function) f, BOOST_THREAD_RV_REF(ArgTypes)...
args) {
...
{
flag.
epoch = uninitialized_flag;
BOOST_VERIFY(!
pthread_cond_broadcast(&thread_detail::once_epoch_cv)
);
BOOST_RETHROW
}
BOOST_CATCH_END
    flag
.
epoch = --thread_detail::once_global_epoch;
BOOST_VERIFY(!
pthread_cond_broadcast(&thread_detail::once_epoch_cv)
);
}
...
{
BOOST_VERIFY(!
pthread_cond_wait(&thread_detail::once_epoch_cv, &thread_detail::once_epoch_mutex
));
}
}
}
