...
class externally_locked {
 ...
 public:
   ...
   *  Move constructor
   */
  externally_locked (BOOST_THREAD_RV_REF(externally_locked)
  rhs) //BOOST_NOEXCEPT
  :
  obj_ (move(rhs
  .obj_)),
  ...
  T &get(strict_lock <mutex_type> &lk) {
    BOOST_THREAD_ASSERT_PRECONDITION(lk.owns_lock(mtx_),
  ...
  T &get(nested_strict_lock <Lock> &lk) {
    BOOST_STATIC_ASSERT((is_same<mutex_type,
                                 typename Lock::mutex_type>::value)); /*< that locks the same type >*/
    BOOST_THREAD_ASSERT_PRECONDITION(lk.owns_lock(mtx_),
                                     lock_error()); /*< run time check throw if not locks the same >*/
    return obj_;
  }
...
template<typename T, typename MutexType>
void
swap(externally_locked<T, MutexType> &lhs, externally_locked<T, MutexType> &rhs) // BOOST_NOEXCEPT
{
  lhs.swap(rhs);
}
