...
struct default_barrier_reseter {
  ...
  default_barrier_reseter(default_barrier_reseter const &other)
  BOOST_NOEXCEPT :
  size_(other.size_) {
  }
  default_barrier_reseter (BOOST_THREAD_RV_REF(default_barrier_reseter)
  other)
  BOOST_NOEXCEPT :
...
struct void_functor_barrier_reseter {
  ...
  void_functor_barrier_reseter(void_functor_barrier_reseter const &other)
  BOOST_NOEXCEPT :
      size_(other.size_), fct_(other
  .fct_)
  {
  }
  void_functor_barrier_reseter (BOOST_THREAD_RV_REF(void_functor_barrier_reseter)
  other)
  BOOST_NOEXCEPT :
      size_(BOOST_THREAD_RV(other).size_), fct_(BOOST_THREAD_RV(other)
  .fct_)
...
struct void_fct_ptr_barrier_reseter {
  ...
  void_fct_ptr_barrier_reseter(void_fct_ptr_barrier_reseter const &other)
  BOOST_NOEXCEPT :
      size_(other.size_), fct_(other
  .fct_)
  {
  }
  void_fct_ptr_barrier_reseter (BOOST_THREAD_RV_REF(void_fct_ptr_barrier_reseter)
  other)
  BOOST_NOEXCEPT :
      size_(BOOST_THREAD_RV(other).size_), fct_(BOOST_THREAD_RV(other)
  .fct_)
...
class barrier {
  static inline unsigned int check_counter(unsigned int count) {
    if (count == 0)
      mars_boost::throw_exception(
          thread_exception(system::errc::invalid_argument,
 ...
 public:
  ...
  barrier(unsigned int count, void(*funct)()) :
      m_count(check_counter(count)), m_generation(0),
      fct_(funct
           ? BOOST_THREAD_MAKE_RV_REF(thread_detail::size_completion_function(
