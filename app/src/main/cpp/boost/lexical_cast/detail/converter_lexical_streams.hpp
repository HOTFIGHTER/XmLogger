...
class basic_unlockedbuf: public basic_pointerbuf<CharT, BufferType> {
 public:
  ...
  typedef BOOST_DEDUCED_TYPENAME base_type
...
template<class CharT // a result of widest_char transformation
    , class Traits, bool RequiresStringbuffer, std::size_t CharacterBufferSize
>
class lexical_istream_limited_src: mars_boost::noncopyable {
  typedef BOOST_DEDUCED_TYPENAME out_stream_helper_trait
  <CharT, Traits>::buffer_t
  buffer_t;

  typedef BOOST_DEDUCED_TYPENAME out_stream_helper_trait
  <CharT, Traits>::out_stream_t
  out_stream_t;

  typedef BOOST_DEDUCED_TYPENAME mars_boost
  ::mpl::if_c<
...
/************************************ HELPER FUNCTIONS FOR OPERATORS << ( ... ) ********************************/
  ...
  inline bool shl_signed(const T n) {
    CharT *tmp_finish = buffer + CharacterBufferSize;
    typedef BOOST_DEDUCED_TYPENAME mars_boost
    ::make_unsigned<T>::type utype;
    CharT *tmp_start =
        lcast_put_unsigned<Traits, utype, CharT>(lcast_to_unsigned(n), tmp_finish).convert();
    if (n < 0) {
      --tmp_start;
...
#endif
                "%.*Lg",
                static_cast<int>(mars_boost::detail::lcast_get_precision<long double>()),
                val);
    return finish > start;
  }
#endif
  ...
  bool shl_real_type(double val, wchar_t *begin) {
    using namespace std;
    finish = start + swprintf(begin,
                              CharacterBufferSize,
                              L"%.*g",
                              static_cast<int>(mars_boost::detail::lcast_get_precision<double>()),
                              val);
    return finish > start;
  }
...
/************************************ OPERATORS << ( ... ) ********************************/
 public:
  ...
  template<class Alloc>
  bool operator<<(mars_boost::container::basic_string<CharT, Traits, Alloc> const &str)
  BOOST_NOEXCEPT {
    start = str.data();
    finish = start + str.length();
    return true;
  ...
  BOOST_DEDUCED_TYPENAME mars_boost::disable_if<mars_boost::is_const<C>, bool>::type
  operator<<(mars_boost::array<C, N> const &input)
  BOOST_NOEXCEPT {
  ...
  bool operator<<(mars_boost::array<const CharT, N> const &input)
  BOOST_NOEXCEPT {
    return shl_char_array_limited(input.begin(), N);
  }

  template<std::size_t N>
  bool operator<<(mars_boost::array<const unsigned char, N> const &input)
  BOOST_NOEXCEPT {
    return ((*this) << reinterpret_cast<mars_boost::array<const char, N> const & >(input));
  }

  template<std::size_t N>
  bool operator<<(mars_boost::array<const signed char, N> const &input)
  BOOST_NOEXCEPT {
    return ((*this) << reinterpret_cast<mars_boost::array<const char, N> const & >(input));
  }
...
/************************************ OPERATORS >> ( ... ) ********************************/
 public:
  ...
  template<std::size_t N>
  bool operator>>(mars_boost::array<CharT, N> &output)
  BOOST_NOEXCEPT {
    return shr_std_array<N>(output);
  }

  template<std::size_t N>
  bool operator>>(mars_boost::array<unsigned char, N> &output)
  BOOST_NOEXCEPT {
    return ((*this) >> reinterpret_cast<mars_boost::array<char, N> & >(output));
  }

  template<std::size_t N>
  bool operator>>(mars_boost::array<signed char, N> &output)
  BOOST_NOEXCEPT {
    return ((*this) >> reinterpret_cast<mars_boost::array<char, N> & >(output));
  }
