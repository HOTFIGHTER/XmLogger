...
inline bool parse_inf_nan_impl(const CharT *begin,
                               const CharT *end,
                               T &value,
                               const CharT *lc_NAN,
                               const CharT *lc_nan,
                               ...
                               const CharT *lc_infinity,
                               const CharT opening_brace,
                               const CharT closing_brace)
BOOST_NOEXCEPT
{
using namespace std;
...
value = (mars_boost::math::changesign) (std::numeric_limits<T>::quiet_NaN());
return true;
} else if (
( /* 'INF' or 'inf' */
end - begin == 3      // 3 == sizeof('inf') - 1
&&
lc_iequal(begin, lc_infinity, lc_INFINITY,
3)
)
||
...
value = (mars_boost::math::changesign) (std::numeric_limits<T>::infinity());
return true;
}
...
template<class CharT, class T>
bool put_inf_nan_impl(CharT *begin,
                      CharT *&end,
                      const T &value,
                      const CharT *lc_nan,
                      const CharT *lc_infinity)
...
BOOST_NOEXCEPT {
return
parse_inf_nan_impl(begin, end, value
,
L"NAN", L"nan"
...
BOOST_NOEXCEPT {
return
put_inf_nan_impl(begin, end, value,
L"nan", L"infinity");
}
