...
exception_detail {
std::string
diagnostic_information_impl(mars_boost::exception const *, std::exception const *, bool, bool);
}
...
inline
std::string
diagnostic_information_impl(mars_boost::exception const *be,
  ...
  if (verbose)
    tmp << std::string("Dynamic exception type: ") <<
        core::demangle((be ? (BOOST_EXCEPTION_DYNAMIC_TYPEID(*be))
                           : (BOOST_EXCEPTION_DYNAMIC_TYPEID(*se))).type_->name()) << '\n';
#endif
...
diagnostic_information(T const &e, bool verbose = true) {
  return exception_detail::diagnostic_information_impl(exception_detail::get_boost_exception(&e),
