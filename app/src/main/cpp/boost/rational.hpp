...
den(is_normalized(int_type(r.numerator()),
                  int_type(r.denominator()))
? r.
denominator() :
...
static BOOST_CONSTEXPR
    int_type
inner_gcd( param_type
a,
param_type b, int_type
const &
zero =
int_type(0)
)
...
// Assign in place
template<typename IntType>
inline rational <IntType> &rational<IntType>::assign(param_type
n,
param_type d
)
{
...
template<typename IntType>
inline rational<IntType> &
    rational<IntType>::operator*=(param_type
i)
{
// Avoid overflow and preserve normalization
...
template<typename IntType>
bool rational<IntType>::operator<(param_type
i) const
{
// Avoid repeated construction
int_type const zero(0);
...
template<typename IntType>
BOOST_CONSTEXPR
inline bool rational<IntType>::operator==(param_type
i) const
{
...
struct gcd_evaluator<rational<IntType> > {
  ...
  result_type operator()(first_argument_type const &a, second_argument_type const &b
  ) const {
