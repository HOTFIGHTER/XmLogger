  ...
  while (true) {
    ...
    if (Bits < 32 && available_bits < 32 - remaining_bits) {
      ...
      if (Bits < 32 || remaining_bits != 0) {
        ...
        mars_boost::uint_least32_t
            extra_bits = mars_boost::uint_least32_t(fmod(val, mult32)) & (divisor - 1);
        val = val / divisor;
  ...
  while (true) {
    ...
    if (available_bits == 32) {
      *begin++ = static_cast<mars_boost::uint_least32_t>(val) & 0xFFFFFFFFu;
      ...
      if (bits < 32 || remaining_bits != 0) {
        mars_boost::uint_least32_t extra_bits = mars_boost::uint_least32_t(val)
            & ((mars_boost::uint_least32_t(1) << (32 - remaining_bits)) - 1);
...
void generate(Engine &eng, Iter first, Iter last) {
  return detail::generate_impl(eng,
                               first,
                               last,
                               mars_boost::random::traits::is_integral<typename Engine::result_type>());
}
...
IntType seed_one_int(SeedSeq &seq) {
  static const int log = ::mars_boost::mpl::if_c<(m == 0),
                                                 ::mars_boost::mpl::int_<(::std::numeric_limits<
                                                     IntType>::digits)>,
...
inline void seed_array_int_impl(SeedSeq &seq, IntType (&x)[n], mars_boost::mpl::true_) {
  BOOST_STATIC_ASSERT_MSG(mars_boost::is_integral<IntType>::value,
                          "Sorry but this routine has not been ported to non built-in integers as it relies on a reinterpret_cast.");
