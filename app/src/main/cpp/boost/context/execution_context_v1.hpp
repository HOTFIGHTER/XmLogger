...
class BOOST_CONTEXT_DECL execution_context{
    ...
    Fn && fn, Args && ... args) {
      ...
      const std::size_t
          size = sctx.size - (static_cast< char * >( sctx.sp) - static_cast< char * >( sp));
#endif
      // create fast-context
      const detail::fcontext_t
          fctx = detail::make_fcontext(sp, size, &execution_context::entry_func < capture_t > );
    ...
    Fn && fn, Args && ... args) {
      ...
      // calculate remaining size
      const std::size_t
          size = palloc.size - (static_cast< char * >( palloc.sp) - static_cast< char * >( sp));
#endif
      // create fast-context
      const detail::fcontext_t
          fctx = detail::make_fcontext(sp, size, &execution_context::entry_func < capture_t > );
      ...
      // placment new for control structure on fast-context stack
      return ::new(sp) capture_t{
          palloc.sctx, salloc, fctx, curr.get(), std::forward<Fn>(fn),
    ...
    // used to extract the function arguments from std::tuple<>
    ptr_{
      create_context(fixedsize_stack(),
                     std::forward<Fn>(fn),
                     std::forward<Args>(args) ...)
    } {
      ptr_->resume(ptr_.get());
...
inline
void swap(execution_context & l, execution_context & r)
noexcept {
l.
swap( r);
}
