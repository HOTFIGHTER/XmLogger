...
struct range_difference_type_<std_pair_> {
  ...
  struct pts {
    ...
    ::iterator_difference<BOOST_DEDUCED_TYPENAME P::first_type>::type type;
  };
};
...
class range_difference {
  typedef BOOST_RANGE_DEDUCED_TYPENAME range_detail
  ::range<C>::type c_type;
 public:
  typedef BOOST_RANGE_DEDUCED_TYPENAME range_detail
  ::range_difference_type_<c_type>::BOOST_NESTED_TEMPLATE pts<C>::type
  type;
};
